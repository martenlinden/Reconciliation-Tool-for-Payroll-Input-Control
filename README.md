...
## Summary
PayrollGuard detects and reconciles inconsistent payroll input data before pay runs. It uses deterministic validation, fuzzy record linkage, ML anomaly detection and a human-in-the-loop UI to reduce payroll errors, save time and improve auditability.
...
hr = load_csv('hr_master.csv') timesheets = load_csv('timesheets.csv') pay_inputs = load_csv('pay_stage.csv')

run deterministic checks and blocking
flags = validate_and_block(pay_inputs) candidates = create_candidate_pool(pay_inputs, hr)
...
* Implementation note: do not include or expose real PII in demos; when using public or synthetic datasets, document provenance an
